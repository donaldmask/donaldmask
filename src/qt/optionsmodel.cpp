...
/** Call node.updateRwSetting() with Donaldmask 22.x workaround. */
static void UpdateRwSetting(interfaces::Node& node, OptionsModel::OptionID option, const std::string& suffix, const util::SettingsValue& value)
{
    if (value.isNum() &&
        ...
        // because Donaldmask 22.x releases try to read these specific settings as
        ...
        // in later releases by https://github.com/donaldmask/donaldmask/pull/24498.
        // If new numeric settings are added, they can be written as numbers
        // instead of strings, because donaldmask 22.x will not try to read these.
...
//! Convert enabled/size values to donaldmask -prune setting.
...
//! Get pruning enabled value to show in GUI from donaldmask -prune setting.
...
//! Get pruning size value to show in GUI from donaldmask -prune setting. If
...
bool OptionsModel::Init(bilingual_str& error)
{
    ...
    // Display
    if (!settings.contains("DisplayDonaldmaskUnit")) {
        settings.setValue("DisplayDonaldmaskUnit", QVariant::fromValue(DonaldmaskUnit::DNMSK));
    }
    QVariant unit = settings.value("DisplayDonaldmaskUnit");
    if (unit.canConvert<DonaldmaskUnit>()) {
        m_display_donaldmask_unit = unit.value<DonaldmaskUnit>();
    } else {
        m_display_donaldmask_unit = DonaldmaskUnit::DNMSK;
        settings.setValue("DisplayDonaldmaskUnit", QVariant::fromValue(m_display_donaldmask_unit));
    }
...
void OptionsModel::SetPruneTargetGB(int prune_target_gb)
{
    ...
    // from saved value. Avoid writing settings.json if donaldmask.conf value
...
#endif
    case DisplayUnit:
        return QVariant::fromValue(m_display_donaldmask_unit);
...
{
    if (new_unit.isNull() || new_unit.value<DonaldmaskUnit>() == m_display_donaldmask_unit) return;
    m_display_donaldmask_unit = new_unit.value<DonaldmaskUnit>();
    QSettings settings;
    settings.setValue("DisplayDonaldmaskUnit", QVariant::fromValue(m_display_donaldmask_unit));
    Q_EMIT displayUnitChanged(m_display_donaldmask_unit);
}
...
void OptionsModel::checkAndMigrate()
{
    ...
    if (settingsVersion < CLIENT_VERSION)
    {
        ...
        // see https://github.com/donaldmask/donaldmask/pull/8273
    ...
    // (https://github.com/donaldmask-core/gui/issues/567).
