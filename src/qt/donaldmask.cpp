...
static void RegisterMetaTypes()
{
    ...
    qRegisterMetaTypeStreamOperators<DonaldmaskUnit>("DonaldmaskUnit");
#else
    qRegisterMetaType<DonaldmaskUnit>("DonaldmaskUnit");
#endif
}
...
static void initTranslations(QTranslator &qtTranslatorBase, QTranslator &qtTranslator, QTranslator &translatorBase, QTranslator &translator)
{
    ...
    // Load e.g. donaldmask_de.qm (shortcut "de" needs to be defined in donaldmask.qrc)
    if (translatorBase.load(lang, ":/translations/")) {
        QApplication::installTranslator(&translatorBase);
    }

    // Load e.g. donaldmask_de_DE.qm (shortcut "de_DE" needs to be defined in donaldmask.qrc)
...
static const char* qt_argv = "donaldmask-qt";

DonaldmaskApplication::DonaldmaskApplication()
...
void DonaldmaskApplication::setupPlatformStyle()
{
    // UI per-platform customization
    // This must be done inside the DonaldmaskApplication constructor, or after it, because
    // PlatformStyle::instantiate requires a QApplication
    std::string platformName;
    platformName = gArgs.GetArg("-uiplatform", DonaldmaskGUI::DEFAULT_UIPLATFORM);
...
#ifdef ENABLE_WALLET
void DonaldmaskApplication::createPaymentServer()
{
    paymentServer = new PaymentServer(this);
}
#endif

bool DonaldmaskApplication::createOptionsModel(bool resetSettings)
{
...
void DonaldmaskApplication::createWindow(const NetworkStyle *networkStyle)
{
    window = new DonaldmaskGUI(node(), platformStyle, networkStyle, nullptr);
    connect(window, &DonaldmaskGUI::quitRequested, this, &DonaldmaskApplication::requestShutdown);
...
void DonaldmaskApplication::createSplashScreen(const NetworkStyle *networkStyle)
{
    assert(!m_splash);
...
void DonaldmaskApplication::createNode(interfaces::Init& init)
{
    assert(!m_node);
...
bool DonaldmaskApplication::baseInitialize()
{
    return node().baseInitialize();
}

void DonaldmaskApplication::startThread()
{
    assert(!m_executor);
    m_executor.emplace(node());

    /*  communication to and from thread */
    connect(&m_executor.value(), &InitExecutor::initializeResult, this, &DonaldmaskApplication::initializeResult);
    connect(&m_executor.value(), &InitExecutor::shutdownResult, this, [] {
        QCoreApplication::exit(0);
    });
    connect(&m_executor.value(), &InitExecutor::runawayException, this, &DonaldmaskApplication::handleRunawayException);
    connect(this, &DonaldmaskApplication::requestedInitialize, &m_executor.value(), &InitExecutor::initialize);
    connect(this, &DonaldmaskApplication::requestedShutdown, &m_executor.value(), &InitExecutor::shutdown);
}

void DonaldmaskApplication::parameterSetup()
{
...
void DonaldmaskApplication::InitPruneSetting(int64_t prune_MiB)
{
    optionsModel->SetPruneTargetGB(PruneMiBtoGB(prune_MiB));
}

void DonaldmaskApplication::requestInitialize()
{
...
void DonaldmaskApplication::requestShutdown()
{
...
void DonaldmaskApplication::initializeResult(bool success, interfaces::BlockAndHeaderTipInfo tip_info)
{
    ...
    if(success) {
        ...
        // donaldmask: URIs or payment requests:
        if (paymentServer) {
            connect(paymentServer, &PaymentServer::receivedPaymentRequest, window, &DonaldmaskGUI::handlePaymentRequest);
            connect(window, &DonaldmaskGUI::receivedURI, paymentServer, &PaymentServer::handleURIOrFile);
...
void DonaldmaskApplication::handleRunawayException(const QString &message)
{
...
void DonaldmaskApplication::handleNonFatalException(const QString& message)
{
...
WId DonaldmaskApplication::getMainWinId() const
{
    if (!window)
        return 0;
...
bool DonaldmaskApplication::event(QEvent* e)
{
...
static void SetupUIArgs(ArgsManager& argsman)
{
    ...
    argsman.AddArg("-uiplatform", strprintf("Select platform to customize UI for (one of windows, macosx, other; default: %s)", DonaldmaskGUI::DEFAULT_UIPLATFORM), ArgsManager::ALLOW_ANY | ArgsManager::DEBUG_ONLY, OptionsCategory::GUI);
}
...
int GuiMain(int argc, char* argv[])
{
    ...
    Q_INIT_RESOURCE(donaldmask);
    Q_INIT_RESOURCE(donaldmask_locale);
    ...
    /// 6-7. Parse donaldmask.conf, determine network, switch to network specific
    ...
    // donaldmask: links repeatedly have their payment requests routed to this process:
    if (WalletModel::isWalletEnabled()) {
