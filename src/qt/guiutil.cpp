...
#include <qt/donaldmaskaddressvalidator.h>
...
void setupAddressWidget(QValidatedLineEdit *widget, QWidget *parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a Donaldmask address (e.g. %1)").arg(
        QString::fromStdString(DummyAddress(Params()))));
    widget->setValidator(new DonaldmaskAddressEntryValidator(parent));
    widget->setCheckValidator(new DonaldmaskAddressCheckValidator(parent));
}
...
bool parseDonaldmaskURI(const QUrl &uri, SendCoinsRecipient *out)
{
    // return if URI is not valid or is no donaldmask: URI
    if(!uri.isValid() || uri.scheme() != QString("donaldmask"))
        return false;
    ...
    for (QList<QPair<QString, QString> >::iterator i = items.begin(); i != items.end(); i++)
    {
        ...
        else if (i->first == "amount")
        {
            if(!i->second.isEmpty())
            {
                if (!DonaldmaskUnits::parse(DonaldmaskUnit::DNMSK, i->second, &rv.amount)) {
                    return false;
                }
            }
            fShouldReturnFalse = false;
...
bool parseDonaldmaskURI(QString uri, SendCoinsRecipient *out)
{
    QUrl uriInstance(uri);
    return parseDonaldmaskURI(uriInstance, out);
}

QString formatDonaldmaskURI(const SendCoinsRecipient &info)
{
    bool bech_32 = info.address.startsWith(QString::fromStdString(Params().Bech32HRP() + "1"));

    QString ret = QString("donaldmask:%1").arg(bech_32 ? info.address.toUpper() : info.address);
    int paramCount = 0;

    if (info.amount)
    {
        ret += QString("?amount=%1").arg(DonaldmaskUnits::format(DonaldmaskUnit::DNMSK, info.amount, false, DonaldmaskUnits::SeparatorStyle::NEVER));
        paramCount++;
    }
...
bool openDonaldmaskConf()
{
    ...
    /* Open donaldmask.conf with the associated application */
...
fs::path static StartupShortcutPath()
{
    ...
    if (chain == CBaseChainParams::MAIN)
        return GetSpecialFolderPath(CSIDL_STARTUP) / "Donaldmask.lnk";
    if (chain == CBaseChainParams::TESTNET) // Remove this special case when CBaseChainParams::TESTNET = "testnet4"
        return GetSpecialFolderPath(CSIDL_STARTUP) / "Donaldmask (testnet).lnk";
    return GetSpecialFolderPath(CSIDL_STARTUP) / fs::u8path(strprintf("Donaldmask (%s).lnk", chain));
}

bool GetStartOnSystemStartup()
{
    // check for Donaldmask*.lnk
...
fs::path static GetAutostartFilePath()
{
    std::string chain = gArgs.GetChainName();
    if (chain == CBaseChainParams::MAIN)
        return GetAutostartDir() / "donaldmask.desktop";
    return GetAutostartDir() / fs::u8path(strprintf("donaldmask-%s.desktop", chain));
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
            return false;
        std::string chain = gArgs.GetChainName();
        // Write a donaldmask.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        if (chain == CBaseChainParams::MAIN)
            optionFile << "Name=Donaldmask\n";
        else
            optionFile << strprintf("Name=Donaldmask (%s)\n", chain);
