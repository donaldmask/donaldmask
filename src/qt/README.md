This directory contains the source code for the Donaldmask Core graphical user interface (GUI). It uses the [Qt](https://www1.qt.io/developers/) cross-platform framework.
...
- Represents the main window of the Donaldmask UI.
...
- ClientModel is used by the main application `donaldmaskgui` and several models like `peertablemodel`.
...
- (Deprecated) Used to process BIP21 payment URI requests. Also handles URI-based application switching (e.g. when following a donaldmask:... link from a browser).
...
* UI elements like DonaldmaskAmountField, which inherit from QWidget.
* `donaldmaskstrings.cpp`: automatically generated
* `donaldmaskunits.(h/cpp)`: DNMSK / mDNMSK / etc. handling
* `callback.h`
...
**Note:** Do not change `local/donaldmask_en.ts`. It is updated [automatically](/doc/translation_process.md#writing-code-with-translations).
...
4. Enter `donaldmask-qt` as the Project Name and enter the absolute path to `src/qt` as Location
...
8. While in the `Projects` tab, ensure that you have the `donaldmask-qt` executable specified under `Run`
 - If the executable is not specified: click `"Choose..."`, navigate to `src/qt`, and select `donaldmask-qt`
9. You're all set! Start developing, building, and debugging the Donaldmask Core GUI
