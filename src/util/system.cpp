...
const char * const DONALDMASK_CONF_FILENAME = "donaldmask.conf";
const char * const DONALDMASK_SETTINGS_FILENAME = "settings.json";
...
bool ArgsManager::ParseParameters(int argc, const char* const argv[], std::string& error)
{
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (key == "-") break; //donaldmask-tx using stdin
...
{
    fs::path settings = GetPathArg("-settings", DONALDMASK_SETTINGS_FILENAME);
    if (settings.empty()) {
        return false;
    }
    if (backup) {
...
{
    // Windows: C:\Users\Username\AppData\Roaming\Donaldmask
    // macOS: ~/Library/Application Support/Donaldmask
    // Unix-like: ~/.donaldmask
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Donaldmask";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // macOS
    return pathRet / "Library/Application Support/Donaldmask";
#else
    // Unix-like
    return pathRet / ".donaldmask";
#endif
#endif
}
...
{
    return GetConfigFile(*this, GetPathArg("-conf", DONALDMASK_CONF_FILENAME));
}
