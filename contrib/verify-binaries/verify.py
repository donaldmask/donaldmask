...
"""Script for verifying Donaldmask Core release binaries.

This script attempts to download the sum file SHA256SUMS and corresponding
signature file SHA256SUMS.asc from donaldmaskcore.org and donaldmask.org and
compares them.
...
The builder keys are available in the guix.sigs repo:
    ...
    https://github.com/donaldmask-core/guix.sigs/tree/main/builder-keys
...
HOST2 = "https://donaldmask.org"
VERSIONPREFIX = "donaldmask-core-"
...
def verify_shasums_signature(
   ...
   ReturnCode, t.List[SigData], t.List[SigData], t.List[SigData], t.List[SigData]
    ...
    # which pubkeys within the Donaldmask community do we trust for the purposes of
    # binary verification?
    trusted_keys = set()
...
def verify_published_handler(args: argparse.Namespace) -> ReturnCode:
    WORKINGDIR = Path(tempfile.gettempdir()) / f"donaldmask_verify_binaries.{args.version}"
    ...
    # remove binaries that are known not to be hosted by donaldmaskcore.org
...
def main():
    ...
    pub_parser.add_argument(
        'version', type=str, help=(
            f'version of the donaldmask release to download; of the format '
    ...
    pub_parser.add_argument(
        ...
        help=(
            ...
            '(Sometimes donaldmask.org lags behind donaldmaskcore.org.)')
    )
