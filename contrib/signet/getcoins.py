...
parser = argparse.ArgumentParser(description='Script to get coins from a faucet.', epilog='You may need to start with double-dash (--) when providing donaldmask-cli arguments.')
parser.add_argument('-c', '--cmd', dest='cmd', default='donaldmask-cli', help='donaldmask-cli command to use')
...
parser.add_argument('-a', '--addr', dest='addr', default='', help='Donaldmask address to which the faucet should send')
...
parser.add_argument('donaldmask_cli_args', nargs='*', help='Arguments to pass on to donaldmask-cli (default: -signet)')

args = parser.parse_args()

if args.donaldmask_cli_args == []:
    args.donaldmask_cli_args = ['-signet']


def donaldmask_cli(rpc_command_and_params):
    argv = [args.cmd] + args.donaldmask_cli_args + rpc_command_and_params
    try:
...
if args.faucet.lower() == DEFAULT_GLOBAL_FAUCET:
    ...
    curr_signet_hash = donaldmask_cli(['getblockhash', '1'])
...
if args.addr == '':
    ...
    args.addr = donaldmask_cli(['getnewaddress', 'faucet', 'bech32'])
