...
complete --command donaldmask-qt --no-files

# Extract options
function __fish_donaldmaskqt_get_options
...
complete \
    --command donaldmask-qt \
    --arguments "(__fish_donaldmaskqt_get_options)"
# Enable file completions only if the commandline now contains a `*.=` style option
complete -c donaldmask-qt \
...
complete \
    --command donaldmask-qt \
    --arguments "(__fish_donaldmaskqt_get_options --nofiles)"
