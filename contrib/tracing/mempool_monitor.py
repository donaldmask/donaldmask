...
""" Example logging Donaldmask Core mempool events using the mempool:added,
...
def main(donaldmaskd_path):
    donaldmaskd_with_usdts = USDT(path=str(donaldmaskd_path))

    # attaching the trace functions defined in the BPF program
    # to the tracepoints
    donaldmaskd_with_usdts.enable_probe(probe="mempool:added", fn_name="trace_added")
    donaldmaskd_with_usdts.enable_probe(probe="mempool:removed", fn_name="trace_removed")
    donaldmaskd_with_usdts.enable_probe(probe="mempool:replaced", fn_name="trace_replaced")
    donaldmaskd_with_usdts.enable_probe(probe="mempool:rejected", fn_name="trace_rejected")
    bpf = BPF(text=PROGRAM, usdt_contexts=[donaldmaskd_with_usdts])
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("USAGE: ", sys.argv[0], "path/to/donaldmaskd")
        exit(1)
