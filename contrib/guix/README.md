# Bootstrappable Donaldmask Core Builds

This directory contains the files necessary to perform bootstrappable Donaldmask
Core builds.
...
To build Donaldmask Core reproducibly with all default options, invoke the
...
2. The `guix-codesign` command ***requires*** a `DETACHED_SIGS_REPO` flag.
    * _**DETACHED_SIGS_REPO**_
      ...
      Donaldmask Core version being built.
...
```
env DETACHED_SIGS_REPO=<path/to/donaldmask-detached-sigs> ./contrib/guix/guix-codesign
```
...
repository](https://github.com/donaldmask-core/guix.sigs).
