...
TMPDIR_PREFIX = "donaldmask_func_test_"
...
class DonaldmaskTestMetaClass(type):
    """Metaclass for DonaldmaskTestFramework.

    Ensures that any attempt to register a subclass of `DonaldmaskTestFramework`
    adheres to a standard whereby the subclass overrides `set_test_params` and
    `run_test` but DOES NOT override either `__init__` or `main`. If any of
    those standards are violated, a ``TypeError`` is raised."""

    def __new__(cls, clsname, bases, dct):
        if not clsname == 'DonaldmaskTestFramework':
            if not ('run_test' in dct and 'set_test_params' in dct):
                raise TypeError("DonaldmaskTestFramework subclasses must override "
                                "'run_test' and 'set_test_params'")
            if '__init__' in dct or 'main' in dct:
                raise TypeError("DonaldmaskTestFramework subclasses may not override "
                                "'__init__' or 'main'")

        return super().__new__(cls, clsname, bases, dct)


class DonaldmaskTestFramework(metaclass=DonaldmaskTestMetaClass):
    """Base class for a donaldmask test script.

    Individual donaldmask test scripts should subclass this class and override the set_test_params() and run_test() methods.
    ...
    def parse_args(self):
        ...
        parser.add_argument("--nocleanup", dest="nocleanup", default=False, action="store_true",
                            help="Leave donaldmaskds and test.* datadir on exit or error")
        ...
        parser.add_argument("--noshutdown", dest="noshutdown", default=False, action="store_true",
                            help="Don't stop donaldmaskds after the test execution")
        ...
        parser.add_argument("--usecli", dest="usecli", default=False, action="store_true",
                            help="use donaldmask-cli instead of RPC for all commands")
    ...
    def setup(self):
        ...
        fname_donaldmaskd = os.path.join(
            config["environment"]["BUILDDIR"],
            "src",
            "donaldmaskd" + config["environment"]["EXEEXT"],
        )
        fname_donaldmaskcli = os.path.join(
            config["environment"]["BUILDDIR"],
            "src",
            "donaldmask-cli" + config["environment"]["EXEEXT"],
        )
        fname_donaldmaskutil = os.path.join(
            config["environment"]["BUILDDIR"],
            "src",
            "donaldmask-util" + config["environment"]["EXEEXT"],
        )
        self.options.donaldmaskd = os.getenv("DONALDMASKD", default=fname_donaldmaskd)
        self.options.donaldmaskcli = os.getenv("DONALDMASKCLI", default=fname_donaldmaskcli)
        self.options.donaldmaskutil = os.getenv("DONALDMASKUTIL", default=fname_donaldmaskutil)
    ...
    def shutdown(self):
        ...
        if not self.options.noshutdown:
            ...
            self.log.info("Note: donaldmaskds were not stopped and may still be running")
        ...
        rpc_logger = logging.getLogger("DonaldmaskRPC")
    ...
    def add_nodes(self, num_nodes: int, extra_args=None, *, rpchost=None, binary=None, binary_cli=None, versions=None):
        ...
        if binary is None:
            binary = [get_bin_from_version(v, 'donaldmaskd', self.options.donaldmaskd) for v in versions]
        if binary_cli is None:
            binary_cli = [get_bin_from_version(v, 'donaldmask-cli', self.options.donaldmaskcli) for v in versions]
        ...
        for i in range(num_nodes):
            test_node_i = TestNode(
                ...
                donaldmaskd=binary[i],
                donaldmask_cli=binary_cli[i],
                version=versions[i],
    ...
    def stop_node(self, i, expected_stderr='', wait=0):
        """Stop a donaldmaskd test node"""
        self.nodes[i].stop_node(expected_stderr, wait=wait)

    def stop_nodes(self, wait=0):
        """Stop multiple donaldmaskd test nodes"""
        for node in self.nodes:
    ...
    def _start_logging(self):
        ...
        ch.setLevel(ll)
        # Format logs the same as donaldmaskd's debug.log with microprecision (so log files can be concatenated and sorted)
    ...
    def _initialize_chain(self):
        ...
        if not os.path.isdir(cache_node_dir):
            ...
            self.nodes.append(
                TestNode(
                    ...
                    timewait=self.rpc_timeout,
                    timeout_factor=self.options.timeout_factor,
                    donaldmaskd=self.options.donaldmaskd,
                    donaldmask_cli=self.options.donaldmaskcli,
                    coverage_dir=None,
                    cwd=self.options.tmpdir,
                    descriptors=self.options.descriptors,
                ))
        ...
        for i in range(self.num_nodes):
            ...
            initialize_datadir(self.options.tmpdir, i, self.chain, self.disable_autoconnect)  # Overwrite port/rpcport in donaldmask.conf
    ...
    def skip_if_no_donaldmaskd_tracepoints(self):
        """Skip the running test if donaldmaskd has not been compiled with USDT tracepoint support."""
        if not self.is_usdt_compiled():
            raise SkipTest("donaldmaskd has not been built with USDT tracepoints enabled.")
    ...
    def skip_if_no_donaldmaskd_zmq(self):
        """Skip the running test if donaldmaskd has not been compiled with zmq support."""
        if not self.is_zmq_compiled():
            raise SkipTest("donaldmaskd has not been built with zmq enabled.")
    ...
    def skip_if_no_wallet_tool(self):
        """Skip the running test if donaldmask-wallet has not been compiled."""
        if not self.is_wallet_tool_compiled():
            raise SkipTest("donaldmask-wallet has not been compiled")

    def skip_if_no_donaldmask_util(self):
        """Skip the running test if donaldmask-util has not been compiled."""
        if not self.is_donaldmask_util_compiled():
            raise SkipTest("donaldmask-util has not been compiled")

    def skip_if_no_cli(self):
        """Skip the running test if donaldmask-cli has not been compiled."""
        if not self.is_cli_compiled():
            raise SkipTest("donaldmask-cli has not been compiled.")
    ...
    def is_cli_compiled(self):
        """Checks whether donaldmask-cli was compiled."""
    ...
    def is_wallet_tool_compiled(self):
        """Checks whether donaldmask-wallet was compiled."""
        return self.config["components"].getboolean("ENABLE_WALLET_TOOL")

    def is_donaldmask_util_compiled(self):
        """Checks whether donaldmask-util was compiled."""
        return self.config["components"].getboolean("ENABLE_DONALDMASK_UTIL")
