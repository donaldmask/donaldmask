# User-space, Statically Defined Tracing (USDT) for Donaldmask Core

Donaldmask Core includes statically defined tracepoints to allow for more
...
**Note**: Donaldmask Core uses temporary clones of the _main_ UTXO cache
...
## Adding tracepoints to Donaldmask Core
...
Multiple tools can list the available tracepoints in a `donaldmaskd` binary with
USDT support.

### GDB - GNU Project Debugger

To list probes in Donaldmask Core, use `info probes` in `gdb`:
...
stap net        inbound_message  0x000000000014419e /src/donaldmaskd
stap net        outbound_message 0x0000000000107c05 /src/donaldmaskd
stap validation block_connected  0x00000000002fb10c /src/donaldmaskd
…
```

### With `readelf`

The `readelf` tool can be used to display the USDT tracepoints in Donaldmask Core.
Look for the notes with the description `NT_STAPSDT`.

```
$ readelf -n ./src/donaldmaskd | grep NT_STAPSDT -A 4 -B 2
