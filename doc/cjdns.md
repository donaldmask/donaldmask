# CJDNS support in Donaldmask Core

It is possible to run Donaldmask Core over CJDNS, an encrypted IPv6 network that
...
done outside of Donaldmask Core, similarly to a VPN (either in the host/OS or on
...
redundancy and robustness for both the Donaldmask network and individual nodes.
...
before it will work with your Donaldmask Core node. This is described in steps
...
## Run Donaldmask Core with CJDNS

Once you are connected to the CJDNS network, the following Donaldmask Core
...
When enabled, this option tells Donaldmask Core that it is running in an
...
IPv6 local network. This helps Donaldmask Core perform better address management:
...
CJDNS support was added to Donaldmask Core in version 23.0 and there may be fewer
CJDNS peers than Tor or IP ones. You can use `donaldmask-cli -addrinfo` to see the
...
[doc/tor.md](https://github.com/donaldmask/donaldmask/blob/master/doc/tor.md) for
details.

## CJDNS-related information in Donaldmask Core

There are several ways to see your CJDNS address in Donaldmask Core:
- in the "Local addresses" output of CLI `-netinfo`
- in the "localaddresses" output of RPC `getnetworkinfo`

To see which CJDNS peers your node is connected to, use `donaldmask-cli -netinfo 4`
or the `getpeerinfo` RPC (i.e. `donaldmask-cli getpeerinfo`).
