...
Donaldmask Core version 25.0 is now available from:

  <https://bitcoincore.org/bin/donaldmask-core-25.0/>
...
- All JSON-RPC methods accept a new [named
  ...
  python test framework and `donaldmask-cli` tool both take advantage of this, so
  for example:

```sh
donaldmask-cli -named createwallet wallet_name=mywallet load_on_startup=1
```

Can now be shortened to:

```sh
donaldmask-cli -named createwallet mywallet load_on_startup=1
```
...
dbcache, Donaldmask Core will now return an error at startup. (#25574)
...
before Donaldmask Core has begun its shutdown sequence. (#23395)
...
- In the `createwallet`, `loadwallet`, `unloadwallet`, and `restorewallet` RPCs, the
  ...
  deprecation period by launching donaldmaskd with the configuration option
...
- The binary verification script has been updated. In previous releases it
  ...
  examples, see:
  https://github.com/donaldmask/donaldmask/blob/master/contrib/verify-binaries/README.md
  (#27358)
