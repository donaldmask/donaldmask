# PSBT Howto for Donaldmask Core

Since Donaldmask Core 0.17, an RPC interface exists for Partially Signed Donaldmask
Transactions (PSBTs, as specified in
[BIP 174](https://github.com/donaldmask/bips/blob/master/bip-0174.mediawiki)).
...
PSBT is an interchange format for Donaldmask transactions that are not fully signed
...
#### Multisig with multiple Donaldmask Core instances
...
Donaldmask Core. We assume their wallets only contain the multisig funds. In case
they also have a personal wallet, this can be accomplished through the
multiwallet feature - possibly resulting in a need to add `-rpcwallet=name` to
the command line in case `donaldmask-cli` is used.
...
Later, when *V* DNMSK has been received on *Amulti*, and Bob and Carol want to
