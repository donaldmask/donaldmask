...
Below are some notes on how to build Donaldmask Core for Windows.

The options known to work for building Donaldmask Core on Windows are:
...
Note that for WSL the Donaldmask Core source path MUST be somewhere in the default mount file system, for
example /usr/src/donaldmask, AND not under /mnt/d/. If this is not the case the dependency autoconf scripts will fail.
...
way. This will install to `c:\workspace\donaldmask`, for example:

    make install DESTDIR=/mnt/c/workspace/donaldmask
